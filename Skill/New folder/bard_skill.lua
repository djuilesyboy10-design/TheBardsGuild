...
API.registerSkill(SKILL_ID, {
    ...
    -- FIX: Removed empty "bgr" string. Only provide "fgr" if no background exists.
    icon = {
        fgr = "icons/swimming/swim.dds",
    },
    ...
    -- FIX: Replaced invalid .Skills with a valid predefined subsection.
    -- Use "Arts" or "Misc" based on your preference.
    statsWindowProps = {
...
return {
    eventHandlers = {
        HerbalismHarvest = function(data)
            ...
            local xpAmount = amount * 0.5
            API.skillUsed(SKILL_ID, { useType = 1, amount = xpAmount })
            print("[Bard XP] Gained " .. xpAmount .. " from herbalism harvest")
        end
    },
    engineHandlers = {
