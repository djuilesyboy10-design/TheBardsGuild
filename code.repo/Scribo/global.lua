...
local function canDuplicate(bookid)
    ...
    if not bookRecord.isScroll then --для копирования книги должна быть как минимум одна полная чернильница
        local inventory = types.Actor.inventory(player)
        local inkwellCount = inventory:countOf(cmn.inkwellID) or 0
        local usageCount = inkwellsUsed[cmn.inkwellID] or 0

        if usageCount > 0 and inkwellCount < 2 then
            player:sendEvent('scrShowMessage', {
                message = msg("notEnoughInkwell")
            })
            return false
        end
    end
...
local function writeBook(data)
    ...
    if isWritableBook(bookRecord.id) then -- если это книга или свиток для записи
        ...
        -- if not bookRecord.isScroll and not hasPages(bookRecord.id) then
        --     return
        -- end
