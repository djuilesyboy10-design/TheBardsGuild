...
-- Helper functions for Merchant Class NPC system
-- Interactive conversation system for Merchant NPCs
...
-- Import the merchant content data from separate file
local merchantContent = require("scripts.engine.plugins.merchant_class.merchant_content_data")
...
local function showChoiceMenu()
    ...
    local choices = {
        {
            ...
            action = function()
                close()
                local content = merchantContent[1]
        ...
        },
        {
            ...
            action = function()
                close()
                local content = merchantContent[2]
                showResult(content.content)
            end
        },
        {
            ...
            action = function()
                close()
                local content = merchantContent[3]
                showResult(content.content)
            end
        },
        {
            ...
            action = function()
                close()
                local content = merchantContent[4]
                showResult(content.content)
            end
        },
        {
            ...
            action = function()
                close()
                local content = merchantContent[5]
                showResult(content.content)
            end
        }
    }
    ...
    table.insert(content, {
        type = ui.TYPE.Text,
        props = {
            text = "What would you like to ask this merchant?",
            textSize = 28,
