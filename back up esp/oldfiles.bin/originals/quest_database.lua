-- quest_database.lua
-- Pure data file: DO NOT add any 'require' lines here.
return {
    ["jmcg_thieves_lore"] = { 
        name = "The Undercover Act: 2", 
        maxIndex = 100, 
        weight = 5 
    },
    ["jmcg_ablle"] = { 
        name = "The Vibrating Earth", 
        maxIndex = 40, 
        weight = 5 
    },
    ["jmcg_ashsinger_echo"] = { 
        name = "The Ash Singers Echo", 
        maxIndex = 60, 
        weight = 10 
    },
    ["jmcg_ballad"] = { 
        name = "St Jiubs Ballad", 
        maxIndex = 100, 
        weight = 5 
    },
    ["jmcg_daisy_lore"] = { 
        name = "History of Daisy", 
        maxIndex = 110, 
        weight = 5 
    },
    ["jmcg_guide_2"] = { 
        name = "The AshLander Guide", 
        maxIndex = 10, 
        weight = 5 
    },
    ["jmcg_hist_lore"] = { 
        name = "The Memory of Sap", 
        maxIndex = 100, 
        weight = 10 
    },
    ["jmcg_imperial_buisness"] = { 
        name = "The Undercover Act: 1", 
        maxIndex = 115, 
        weight = 10 
    },
    ["jmcg_orc_diplomacy"] = { 
        name = "Orc Diplomacy", 
        maxIndex = 100, 
        weight = 15 
    },
    ["jmcg_path_bad"] = { 
        name = "Slaving all day", 
        maxIndex = 10, 
        weight = 5 
    },
    ["jmcg_path_good"] = { 
        name = "The Abolitionist", 
        maxIndex = 10, 
        weight = 5 
    },
    ["jmcg_recruitment"] = { 
        name = "Recruitment Drive", 
        maxIndex = 100, 
        weight = 10 
    },
    ["jmcg_shane_lore"] = { 
        name = "The Pig Children", 
        maxIndex = 100, 
        weight = 5 
    },
    ["jmcg_shane_lore_1"] = { 
        name = "Orc true nature?", 
        maxIndex = 100, 
        weight = 5 
    },
    ["jmcg_shane_lore_2"] = { 
        name = "Gaze of Malacath", 
        maxIndex = 60, 
        weight = 10 
    },
    ["jmcg_the_chroniclers_saint"] = { 
        name = "The War of the Sky's", 
        maxIndex = 90, 
        weight = 15 
    },
    ["jmcg_the_family_ties"] = { 
        name = "Family Ties", 
        maxIndex = 100, 
        weight = 10 
    },
    ["jmcg_tong_dirge"] = { 
        name = "The Shadow’s Duet", 
        maxIndex = 100, 
        weight = 10 
    },
    ["jmcg_twin_lamps_hymn"] = { 
        name = "Song's of Freedom", 
        maxIndex = 100, 
        weight = 5 
    },

    -- FIXED: safe, canonical quest ID (no spaces or punctuation)
    ["jmcg_bards_guild"] = { 
        name = "The New Bards Guild", 
        maxIndex = 130, 
        weight = 20 
    },

    ["jmcg_joseph_personal"] = { 
        name = "Joseph's Past", 
        maxIndex = 100, 
        weight = 15 
    },
    ["jmcg_tuning_master"] = { 
        name = "Resonance of the Fork", 
        maxIndex = 100, 
        weight = 10 
    },
}
