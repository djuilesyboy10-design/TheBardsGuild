...
-- Register skill stat change handler for real-time updates
API.addSkillStatChangedHandler(function(skillId, oldStat, newStat)
    if skillId == SKILL_ID and oldStat.modified ~= newStat.modified then
        applyBardPassives()
    end
end)
...
local function grantBardXP(amount, reason)
    ...
    print(string.format("[Bard XP] Gained %.3f XP from %s", amount, reason))
end
