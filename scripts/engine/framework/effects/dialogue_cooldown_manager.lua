...
local cooldownStorage = I.S3ProtectedTable.new({
    logPrefix = '[S3 Cooldown Manager]',
    inputGroupName = 'DialogueEffectsCooldowns'
})

-- Get the underlying storage group for direct access
local cooldownSettings = require('openmw.storage').playerSection('DialogueEffectsCooldowns')
...
function cooldownManager.canApplyEffect(effectId)
    local lastTime = cooldownSettings:get(effectId) or 0
...
function cooldownManager.applyEffect(effectId)
    if cooldownManager.canApplyEffect(effectId) then
        ...
        cooldownSettings:set(effectId, core.getGameTime())
        ...
        if success then
            ...
            cooldownSettings:set(effectId, nil)
...
-- Return the cooldown manager interface
