-- noble_class_helper.lua
-- Helper functions for Noble Class NPC system
-- Interactive conversation system for Noble NPCs
...
-- Import the noble content data from separate file
local nobleContent = require("scripts.engine.plugins.noble_class.noble_content_data")
...
local function showChoiceMenu()
    ...
    local choices = {
        {
            text = "Tell me about your family's history",
            action = function()
                close()
                local content = nobleContent[1]
                showResult(content.content)
            end
        },
        {
            text = "What's your position in House politics?",
            action = function()
                close()
                local content = nobleContent[2]
                showResult(content.content)
            end
        },
        {
            text = "Share your thoughts on the Empire's rule",
            action = function()
                close()
                local content = nobleContent[3]
                showResult(content.content)
            end
        },
        {
            text = "What luxuries do you most enjoy?",
            action = function()
                close()
                local content = nobleContent[4]
                showResult(content.content)
            end
        },
        {
            text = "Any advice for an aspiring commoner?",
            action = function()
                close()
                local content = nobleContent[5]
                showResult(content.content)
            end
        }
    }
    ...
    table.insert(content, {
        type = ui.TYPE.Text,
        props = {
            text = "What would you like to ask this noble?",
            textSize = 28,
