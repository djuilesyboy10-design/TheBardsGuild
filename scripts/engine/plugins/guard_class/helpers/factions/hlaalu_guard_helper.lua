...
-- Helper functions for Hlaalu Guard Class NPC system
-- Interactive conversation system for Hlaalu Guard NPCs
...
-- Import the Hlaalu guard content data from separate file
local hlaaluGuardContent = require("scripts.engine.plugins.guard_class.content.factions.hlaalu_guard_content_data")
        ...
        if line == "" then
            ...
            table.insert(loreContent, {
                ...
                props = {
                    text = line,
                    textSize = 28,
                    textColor = util.color.rgb(1.0, 0.8, 0.2), -- Gold color for Hlaalu
                }
            })
        end
    end
...
local function showChoiceMenu()
    ...
    local choices = {
        ...
        },
        {
            ...
            action = function()
                close()
                local content = hlaaluGuardContent[2]
                showResult(content.content)
            end
        },
        {
            ...
            action = function()
                close()
                local content = hlaaluGuardContent[3]
                showResult(content.content)
            end
        },
        {
            ...
            action = function()
                close()
                local content = hlaaluGuardContent[4]
                showResult(content.content)
            end
        },
        {
            ...
            action = function()
                close()
                local content = hlaaluGuardContent[5]
                showResult(content.content)
            end
        }
    }
    ...
    table.insert(content, {
        type = ui.TYPE.Text,
        props = {
            text = "What would you like to ask this Hlaalu guard?",
            textSize = 28,
            textColor = util.color.rgb(1.0, 0.8, 0.2), -- Gold color for Hlaalu
        }
    })
